...
void uwb_get_hw_stats(uwb_hw_stats_t* out)
{
    ...
    _Static_assert(sizeof(uwb_hw_stats_t) == sizeof(uwb_driver_statistics_t),
                   "uwb_hw_stats_t and uwb_driver_statistics_t must remain structurally identical");
    uwb_driver_statistics_t drv = uwb_driver_get_statistics();
    memcpy(out, &drv, sizeof(uwb_hw_stats_t));
}
