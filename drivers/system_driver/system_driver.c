...
STATIC system_driver_device_info_t device_info = {0};
...
void system_driver_get_uuid(uint8_t uuid[SYSTEM_DRIVER_UUID_SIZE])
{
    ...
    memcpy(uuid, uid, SYSTEM_DRIVER_UUID_SIZE);
}
...
void system_driver_device_init(void)
{
    ...
    system_driver_get_uuid(device_info.uuid);
    device_info.uuid_word0 = system_driver_get_uuid_word(0);
    device_info.uuid_word1 = system_driver_get_uuid_word(1);
    device_info.uuid_word2 = system_driver_get_uuid_word(2);

    const device_mapping_entry_t* mapping =
        device_mapping_find(device_info.uuid_word0, device_info.uuid_word1, device_info.uuid_word2);
...
bool system_driver_device_get_info(system_driver_device_info_t* info)
{
    if (info == NULL || !device_initialized)
        return false;
    memcpy(info, &device_info, sizeof(system_driver_device_info_t));
    return true;
}
