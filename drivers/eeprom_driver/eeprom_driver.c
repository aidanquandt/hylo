...
/*---------------------------------------------------------------------------
 * Private Helpers (single-byte; used by buffer API)
 *---------------------------------------------------------------------------*/
static eeprom_status_e eeprom_write_u8(uint16_t reg_addr, uint8_t value)
{
	...
	i2c_driver_status_E status =
		i2c_driver_write_register_16(I2C_EEPROM, M24C64_I2C_ADDR_7BIT, reg_addr, &value, 1U);
...
static eeprom_status_e eeprom_read_u8(uint16_t reg_addr, uint8_t* value)
{
	...
	i2c_driver_status_E status =
		i2c_driver_read_register_16(I2C_EEPROM, M24C64_I2C_ADDR_7BIT, reg_addr, value, 1U);
...
eeprom_status_e eeprom_write_buffer(uint16_t start_reg_addr, const uint8_t* data, uint16_t len)
{
	...
	{
		eeprom_status_e status = eeprom_write_u8(
			(uint16_t)(start_reg_addr + i), data[i]);
...
eeprom_status_e eeprom_driver_read_buffer(uint16_t start_reg_addr, uint8_t* data, uint16_t len)
{
	...
	for (uint16_t i = 0U; i < len; i++)
	{
		eeprom_status_e status = eeprom_read_u8(
			(uint16_t)(start_reg_addr + i), &data[i]);
