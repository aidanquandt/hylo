...
STATIC uwb_driver_status_t uwb_driver_send_message(uwb_dev_t* dev, const uint8_t* data,
                                                    uint16_t length);
STATIC void uwb_driver_handle_irq(void);
STATIC uint32_t uwb_driver_read_irq_status(void);
STATIC uint32_t uwb_driver_read_status_register_low(void);
STATIC uint32_t uwb_driver_read_status_register_high(void);
...
void uwb_driver_register_isr_callbacks(uwb_dev_t* dev)
{
    ...
    if (rx_queue == NULL)
    {
        ...
        (void)xTaskCreate(uwb_driver_irq_task, "uwb_irq", TASK_STACK_4KB, NULL,
                          TASK_PRIORITY_UWB_IRQ, &irq_task_handle);
        (void)xTaskCreate(uwb_driver_rx_task, "uwb_rx", TASK_STACK_3KB, NULL,
                          TASK_PRIORITY_UWB_RX, NULL);
        (void)xTaskCreate(uwb_driver_tx_task, "uwb_tx", TASK_STACK_2KB, NULL,
