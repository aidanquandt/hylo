 ...
 static void sdcard_lowlevel_test(void)
{
    ...
    {
        //uart_driver_printf(UART_CONSOLE, "[SDCard] HAL_SD_GetCardInfo failed\r\n");
        return;
    }

    uint64_t capacity =
        (uint64_t)cardInfo.BlockNbr * cardInfo.BlockSize;
    capacity_mb = (unsigned long)(capacity / (1024ULL * 1024ULL));

    //uart_driver_printf(UART_CONSOLE, "[SDCard] block size: %lu\r\n", (unsigned long)cardInfo.BlockSize);
    //uart_driver_printf(UART_CONSOLE, "[SDCard] block count: %lu\r\n", (unsigned long)cardInfo.BlockNbr);
    //uart_driver_printf(UART_CONSOLE, "[SDCard] capacity: %lu MiB\r\n", capacity_mb);
    //uart_driver_printf(UART_CONSOLE, "[SDCard] card type: %d\r\n", (int)cardInfo.CardType);
}
...
static bool sdcard_mount_and_open(void)
{
    ...
    if (result != FR_OK)
    {
        ...
        uart_driver_printf(UART_CONSOLE, "[SDCard] mount failed (FRESULT=%d)\r\n", (int)result);
        return false;
    }
    ...
    if (result != FR_OK)
    {
        ...
        sdcard_mounted = false;
        uart_driver_printf(UART_CONSOLE, "[SDCard] open file failed (FRESULT=%d)\r\n", (int)result);
        return false;
    }
    ...
    if (result != FR_OK)
    {
        ...
        file_open = false;
        sdcard_mounted = false;
        uart_driver_printf(UART_CONSOLE, "[SDCard] seek failed (FRESULT=%d)\r\n", (int)result);
        return false;
    }

    rows_since_sync = 0;

    uart_driver_printf(UART_CONSOLE, "[SDCard] mounted and file opened\r\n");
    return true;
}
...
static void sdcard_write_csv_header(void)
{
    ...
    {
        file_open = false;
        //uart_driver_printf(UART_CONSOLE, "[SDCard] failed to write header\r\n");
    }
}
...
static void sdcard_write_event(const sdcard_driver_event_t* event)
{
    ...
    {
        file_open = false;
        //uart_driver_printf(UART_CONSOLE, "[SDCard] write failed\r\n");
    }
    ...
    if (rows_since_sync >= SDCARD_SYNC_INTERVAL)
    {
        ...
        {
            file_open = false;
            //uart_driver_printf(UART_CONSOLE, "[SDCard] sync failed\r\n");
        }
        rows_since_sync = 0;
    }
}
