...
typedef struct
{
    ...
    void (*onEntry)(uint16_t prevState); // state on entry function, run once when transitioning
                                         // from another state
    void (*onExit)(
        uint16_t nextState); // state on exit function, run once when transitioning to another state
} state_s;
...
typedef struct
{
    ...
    uint16_t (*transitionLogic)(uint16_t currentState,
                                uint32_t stateTimer); // transition logic function pointer
