...
STATIC bool tag_send_poll(void)
{
    ...
    uwb_send_result_t result =
        uwb_send_message((uint8_t*)&poll, sizeof(poll), tag_ctx.target_address);
    if (!result.success)
    {
    ...
    // Store message ID for TX callback validation
    tag_ctx.pending_tx_id = result.message_id;
    return true;
}
...
STATIC bool tag_send_final(void)
{
    ...
    uwb_send_result_t result =
        uwb_send_message((uint8_t*)&final, sizeof(final), tag_ctx.target_address);
    if (!result.success)
    {
...
STATIC uint16_t tag_transition_logic(uint16_t currentState, uint32_t stateTimer)
{
    switch (currentState)
    {
        ...
        case TAG_STATE_WAIT_FINAL_ACK:
            ...
            if (stateTimer >= TAG_RESPONSE_TIMEOUT_TICKS)
            {
                error_handler_log(ERROR_SEVERITY_ERROR, "tag", "Timeout in state %u", currentState);
                tag_ctx.fault_code = TAG_FAULT_TIMEOUT;
                return TAG_STATE_FAULTED;
            }
            break;
...
bool tag_start_ranging(uint16_t anchor_addr)
{
    ...
    if (tag_sm.curr_state != TAG_STATE_IDLE)
    {
...
{
    return (tag_sm.curr_state != TAG_STATE_IDLE);
}
...
{
    // Validate message ID matches our pending transmission
    if (message_id != tag_ctx.pending_tx_id)
    {
        error_handler_log(ERROR_SEVERITY_WARNING, "tag",
                          "TX callback with wrong message ID (got %u, expected %u)",
                          (unsigned int)message_id, (unsigned int)tag_ctx.pending_tx_id);
        return;
    }
    ...
    if (tag_sm.curr_state == TAG_STATE_SENDING_POLL)
    {
        ...
        tag_ctx.pending_tx_id         = 0; // Clear pending ID
    ...
    {
        error_handler_log(ERROR_SEVERITY_WARNING, "tag", "Unexpected TX callback in state %u",
                          tag_sm.curr_state);
        tag_ctx.pending_tx_id = 0;
    }
}
