/**
 * @file kalman_core.c
 * @brief Extended Kalman Filter core implementation for IMU/UWB sensor fusion
 *
 * Adapted from Crazyflie firmware kalman_core.c
 * Original authors: Michael Hamer (http://www.mikehamer.info), June 2016
 *                   Mark Mueller (www.mwm.im)
 ...
 *---------------------------------------------------------------------------*/
static void predictDt(kalmanCoreData_t* kf, const kalmanCoreParams_t* params,
                      Axis3f* acc, Axis3f* gyro, float dt);
                       ...
                       Axis3f* acc, Axis3f* gyro, uint32_t nowMs)
{
    ...
    if (dt > 0.0f && dt < 1.0f) {  /* Sanity check on dt */
...
void kalmanCoreAddProcessNoise(kalmanCoreData_t* kf, const kalmanCoreParams_t* params, uint32_t nowMs)
{
...
void kalmanCoreScalarUpdate(kalmanCoreData_t* kf, arm_matrix_instance_f32* Hm, 
...
void kalmanCoreUpdateWithPKE(kalmanCoreData_t* kf, arm_matrix_instance_f32* Hm,
                             arm_matrix_instance_f32* Km, arm_matrix_instance_f32* P_w_m, float error)
{
                      ...
                      Axis3f* acc, Axis3f* gyro, float dt)
{
    ...
    kf->S[KC_STATE_PX] += dt * (acc->x + gyro->z * tmpSPY - gyro->y * tmpSPZ 
                                  - GRAVITY_MAGNITUDE * kf->R[2][0]);
    kf->S[KC_STATE_PY] += dt * (acc->y - gyro->z * tmpSPX + gyro->x * tmpSPZ 
                                  - GRAVITY_MAGNITUDE * kf->R[2][1]);
    kf->S[KC_STATE_PZ] += dt * (acc->z + gyro->y * tmpSPX - gyro->x * tmpSPY 
                                  - GRAVITY_MAGNITUDE * kf->R[2][2]);
