...
imu_port_status_t imu_port_probe_and_init(imu_dev_t* dev)
{
    ...
    // CRITICAL: Current IMU (BMI323) requires a RISING EDGE on CSB after power-up to select SPI
    // mode Workaround: Force CS LOW briefly, then HIGH to create rising edge This assumes the
    // device has been powered up long enough (called after 2s delay in test module)
    ...
    platform_os_delay_us_blocking(
        500); // Wait for mode switch (datasheet: 200µs min, use 500µs for safety)
